...
import { MarriageRegistrationForm, MarriageRegistrationFormData } from "./MarriageRegistrationForm";
...
export default function MarriageRegistrationsTable() {
  ...
  const fetchRegistrations = async () => {
    try {
      const response = await api.get(`/registration/marriage?page=${currentPage}&pageSize=10`);
  ...
  const handleEditClick = (reg: MarriageRegistration) => {
    setSelectedRegistration(reg);
    setIsFormOpen(true);
  };

  const handleDeleteClick = (reg: MarriageRegistration) => {
    setSelectedRegistration(reg);
    setIsDeleteOpen(true);
  };

  const handleFormSubmit = async (data: MarriageRegistrationFormData) => {
    try {
      setIsSubmitting(true);
      if (selectedRegistration) {
        await api.put(`/registration/marriage/${selectedRegistration._id}`, data);
      } else {
        const submitData = {
          ...data,
          registrationDateBs: new Date().toISOString().split('T')[0]
        };
        await api.post("/registration/marriage", submitData);
      }
      setIsFormOpen(false);
      fetchRegistrations();
    } catch (err: any) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsSubmitting(true);
      await api.delete(`/registration/marriage/${selectedRegistration._id}`);
  ...
  const columnHelper = createColumnHelper<MarriageRegistration>();
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor("groomName", {
      header: "Groom Name",
      cell: (info) => info.getValue(),
    }),
    columnHelper.accessor("brideName", {
      header: "Bride Name",
      cell: (info) => info.getValue(),
    }),
    columnHelper.accessor("marriageDateBs", {
      header: "Marriage Date (BS)",
      cell: (info) => info.getValue(),
    }),
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/5 dark:bg-white/3">
      ...
      <FormModal
        ...
        title={selectedRegistration ? "Edit Marriage Registration" : "New Marriage Registration"}
      >
