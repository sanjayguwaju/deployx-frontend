...
import { DeathRegistrationForm, DeathRegistrationFormData } from "./DeathRegistrationForm";
...
export default function DeathRegistrationsTable() {
  ...
  const fetchRegistrations = async () => {
    try {
      const response = await api.get(`/registration/death?page=${currentPage}&pageSize=10`);
  ...
  const handleEditClick = (reg: DeathRegistration) => {
    setSelectedRegistration(reg);
    setIsFormOpen(true);
  };

  const handleDeleteClick = (reg: DeathRegistration) => {
    setSelectedRegistration(reg);
    setIsDeleteOpen(true);
  };

  const handleFormSubmit = async (data: DeathRegistrationFormData) => {
    try {
      setIsSubmitting(true);
      if (selectedRegistration) {
        await api.put(`/registration/death/${selectedRegistration._id}`, data);
      } else {
        const submitData = {
          ...data,
          registrationDateBs: new Date().toISOString().split('T')[0]
        };
        await api.post("/registration/death", submitData);
      }
      setIsFormOpen(false);
      fetchRegistrations();
    } catch (err: any) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      setIsSubmitting(true);
      await api.delete(`/registration/death/${selectedRegistration._id}`);
  ...
  const columnHelper = createColumnHelper<DeathRegistration>();
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor("dateOfDeathBs", {
      header: "Date of Death (BS)",
      cell: (info) => info.getValue(),
    }),
    columnHelper.accessor("ageAtDeath", {
      header: "Age at Death",
      cell: (info) => info.getValue() || "-",
    }),
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/5 dark:bg-white/3">
      ...
      <FormModal
        ...
        title={selectedRegistration ? "Edit Death Registration" : "New Death Registration"}
      >
