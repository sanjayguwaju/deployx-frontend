...
export default function StatisticsChart() {
  ...
  const [seriesData, setSeriesData] = useState<{name: string, data: number[]}[]>([
    { name: "Complaints", data: Array(12).fill(0) },
    { name: "Service Requests", data: Array(12).fill(0) },
  ]);
    ...
    const fetchSummary = async () => {
      try {
        ...
        if (response.data.success) {
          const { complaints, serviceRequests } = response.data.data;
          setSeriesData([
            { name: "Complaints", data: complaints.monthly || Array(12).fill(0) },
            { name: "Service Requests", data: serviceRequests.monthly || Array(12).fill(0) },
          ]);
        }
      } catch (error) {
        console.error("Failed to fetch dashboard summary for chart:", error);
      }
    };
    fetchSummary();
  }, []);
