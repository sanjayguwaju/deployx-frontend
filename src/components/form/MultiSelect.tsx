      ...
      <Popover.Root open={isOpen} onOpenChange={(open) => {
        ...
        <Popover.Trigger asChild>
          <button
            type="button"
            disabled={disabled}
            className={`flex min-h-11 w-full items-center justify-between rounded-lg border border-gray-300 py-1.5 pl-3 pr-3 shadow-theme-xs outline-none transition focus:border-brand-300 focus:shadow-focus-ring dark:border-gray-700 dark:bg-gray-900 dark:focus:border-brand-300 ${
          ...
          >
            ...
            <div className="flex items-center self-start py-1 pl-1 pr-1 w-7">
              <svg
                className={`stroke-current transition-transform text-gray-700 dark:text-gray-400 ${isOpen ? "rotate-180" : ""}`}
        ...
        <Popover.Portal>
          <Popover.Content
            align="start"
            className="z-10050 w-(--radix-popover-trigger-width) max-h-[300px] overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 dark:border-gray-800 dark:bg-gray-900"
            sideOffset={4}
          >
            <Command className="w-full h-full" loop>
              <Command.List className="max-h-[300px] overflow-y-auto">
                <Command.Empty className="p-4 text-center text-sm text-gray-500">No options found.</Command.Empty>
                <Command.Group>
                  {options.map((option) => {
                    ...
                    return (
                      <Command.Item
                        ...
                        className={`hover:bg-primary/5 w-full cursor-pointer rounded-t border-b border-gray-200 dark:border-gray-800 outline-none data-[selected=true]:bg-gray-50 dark:data-[selected=true]:bg-white/5 ${
